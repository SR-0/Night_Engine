...
void Render::execute()
{
	...
	for (unsigned int renderLayer = 1; renderLayer < *m_RenderLayerCount + 1; renderLayer++)
	{
		...
		for (unsigned int position = 0; position < m_RectangleShapes->objectCount(); position++)
		{
			...
			if (m_RectangleShapes->get(position).isRenderEnabled() && (unsigned int)m_RectangleShapes->get(position).getRenderLayer() == renderLayer)
			{
		...
		for (unsigned int position = 0; position < m_RectangleShapes->objectCount(); position++)
		{
			...
			if (m_Texts->get(position).isRenderEnabled() && (unsigned int)m_Texts->get(position).getRenderLayer() == renderLayer)
			{
